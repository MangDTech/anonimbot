...
@ensure_connections
def reg_db(conn, user_id: int, gender: str, change: str):  # Добавление пользователя в таблицу users
    c = conn.cursor()
    c.execute('INSERT INTO users (user_id, gender, change) VALUES (?,?,?)',
              (user_id, gender, change))
    conn.commit()


@ensure_connections
def edit_db(conn, user_id: int, gender: str, change: str):  # Пересоздание пользователя по user_id в таблицу users
    c = conn.cursor()
    c.execute('UPDATE users SET gender=?,change=? WHERE user_id = ?', (gender, change, user_id))
    conn.commit()
